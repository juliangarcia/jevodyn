...
public class AgentBasedPopulationImplTest {
	...
	public void testVarious() {
		...
		int[] primitiveArray = {0,1,1,1,0}; // 5 individuals, 3 defect 2 cooperate
 		Agent[] array = new Agent[5];
		for (int i = 0; i < array.length; i++) {
			array[i] = new SimpleAgent(primitiveArray[i]); 
		}
		AgentBasedPopulationImpl population = new AgentBasedPopulationImpl(array);
		//done
		
		//changes to the array do not change the population
		array[0] = new SimpleAgent(1); //we set agent 0 to strategy 1 in the underlying array 
		Assert.assertEquals(0, ((SimpleAgent)population.getAgent(0)).getStrategy()); //but agent 0 remains 0 in the population
		...
		Assert.assertEquals(1, ((SimpleAgent)population.getAgent(0)).getStrategy());
 		Assert.assertEquals(1, ((SimpleAgent)population.getAgent(1)).getStrategy());
		Assert.assertEquals(1, ((SimpleAgent)population.getAgent(2)).getStrategy());
		Assert.assertEquals(1, ((SimpleAgent)population.getAgent(3)).getStrategy());
		Assert.assertEquals(0, ((SimpleAgent)population.getAgent(4)).getStrategy());
		//size must be 5
		...
		Assert.assertEquals(1, ((SimpleAgent)population.getAgent(0)).getStrategy());
 		Assert.assertEquals(1, ((SimpleAgent)population.getAgent(1)).getStrategy());
		Assert.assertEquals(2, ((SimpleAgent)population.getAgent(2)).getStrategy());
		Assert.assertEquals(1, ((SimpleAgent)population.getAgent(3)).getStrategy());
		Assert.assertEquals(0, ((SimpleAgent)population.getAgent(4)).getStrategy());
